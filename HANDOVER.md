...
## P4. 筹码峰演化过程指标系统 (v3, 2026-06-30 新增)

> **背景**：用户分析方法一贯侧重"演化过程"而非"静态快照"。之前的 v1（仅 7 日差分）和 v2（全静态截面）均不足以描述筹码结构随时间的完整变化。v3 补齐 7 项时间序列维度的演化特征，并加权为 0~100 的 v3 演化评分。

### 4.1 设计原则

所有 v3 指标必须满足"**同一指标 ≥ 2 天历史**才能计算"，坚决排除任何静态截面。

- **v1 chip_score**：7日差分 → 仅覆盖 25% 演化维度（TPC↑/ width↓/ winner↑/ TP3↑）
- **v2 chip_score_v2**：全静态截面 → 0% 演化
- **v3 chip_score_evolution**：7 项时间序列 → **100% 演化过程**

### 4.2 七项演化指标定义

| # | 指标字段 | 单位 | 说明 | 高分含义 |
|---|:---|:---:|:---|:---|
| 1 | `peak_shift_14d` | % | 14天主峰价格相对变化（今日 vs 14日前）| **+10% → 主力上移吸筹（好）**；-5% → 主峰下移派发 |
| 2 | `peak_shift_streak` | 天 | 主峰价格连续同方向移动的天数（反转为 0） | ≥4 → 方向明确，非随机波动 |
| 3 | `tpc_streak` | 天 | TPC（前三峰集中度）连续 ≥ 15% 的天数 | ≥10 → 筹码长期集中，主力未撤 |
| 4 | `conv_path_score` | 分 | 近14天形态转移累计得分：收敛（多峰→密集）+2/天；发散（密集→多峰）-2/天 | 正值→**筹码正在收拢吸筹**；负值→正在发散派发 |
| 5 | `dense_days` | 天 | 当前形态连续为"密集类"（单峰密集/双峰密集）的天数 | ≥5 → 主力已横盘吸筹完毕，随时启动 |
| 6 | `res_melt_7d` | % | 7日上方阻力峰筹码量消融速率 = (7日前阻峰筹码量 - 今日阻峰)/7日前 | **正值+20% → 上方筹码被吃掉，即将突破**；负值→阻力反而加重 |
| 7 | `evol_consistency` | ±3 | 3日/7日/14日 v1 score 方向一致性，3窗口全升 +3，全降 -3 | ≥+2 → **各周期信号共振**；≤-2 → 明确派发 |

### 4.3 v3 综合评分公式 (chip_score_evolution)

```
v3 = 20% × norm(peak_shift_14d, -5%, +10%)
   + 10% × norm(peak_shift_streak, 0, 7)
   + 20% × norm(tpc_streak,         0, 15)
   + 15% × norm(conv_path_score,   -5, +8)
   + 10% × norm(dense_days,         0, 20)
   + 10% × norm(res_melt_7d,      -30%, +60%)
   + 15% × norm(evol_consistency,  -3, +3)
   → 归一化 0~100 分
```
...
| ≥48 | 演化中 (弱) | 黄 | 🟡 中性观察 |
| ≥32 | 演化转弱 | 红 | 🔴 不新开仓 |
| <32 | 演化走弱 (派发) | 红 | 🔴 建议减仓/清仓 |

### 4.5 代码位置 / 调用方式

| 函数 | 位置 |
|:---|:---|
| `compute_evolution_metrics(sorted_daily_results, i, windows=(3,7,14))` | [chip_indicators.py](file:///home/xiangan/stocks/stock-chip-quant/chip_indicators.py#L592-L729) |
| `chip_score_evolution(metrics_dict) → float [0,100]` | [chip_indicators.py](file:///home/xiangan/stocks/stock-chip-quant/chip_indicators.py#L732-L784) |
| `evolution_grade(v3_score) → (label, color_class)` | [chip_indicators.py](file:///home/xiangan/stocks/stock-chip-quant/chip_indicators.py#L787-L793) |

### 4.6 compute_all_chip_metrics 已自动注入

从 2026-06-30 起，`compute_all_chip_metrics()` 返回的每条 `result` 会自动包含：
...
```python
{
  ...
  'score': 5, 'v2': 75.4, ...
  # 新增 7 项演化指标
  'peak_shift_14d': +12.8, 'peak_shift_streak': 3,
  'tpc_streak': 82, 'conv_path_score': +5.8,
  'dense_days': 5, 'res_melt_7d': +18.3,
  'evol_consistency': -1,
  # 新增演化综合评分
  'v3': 65.3,
}
...
历史不足的日期上述字段为 None/0，`v3 = None`。

### 4.7 2026-06-30 首次落地验证（20 只持仓+关注）

**最关键发现**：v1 与 v3 在 20 只中 **13 只有分歧**。之前 v1 判为"多"的多只票，v3 实际上已在派发末期：

| 股票 | v1 静态快照 | v3 演化过程 | 实际含义 |
|:---|:---:|:---:|:---|
| **TCL科技** | 多 | **多** (65.3) | ✅ 唯一真双强，形态得分 +5.8（收敛中）+ 阻融 +18%（正在突破）|
| 立昂微 | 多 | 中 (59.7) | 快照好但演化走平，需警惕拐点 |
| 海思科 | 多 | **空** (41.0) | ⚠️ v1=6 满分，v3=41。演化显示 conv_path=-3.9（形态已发散）|
| 美埃科技 | 多 | **空** (31.0) | ⚠️ 静态 v1=6，但 tpc_streak=0（TPC<15%），筹码散 + 演化派发 |
| 中国长城 | 多 | **空** (43.3) | ⚠️ v1=7，但 conv_path=-4.7（发散中），阻融 -770%（阻力极大加重）|
| 恒瑞医药 | 多 | **空** (39.7) | 主峰 shift -12.5%（向下派发）|
| 璞泰来 | 空 | **派发** (11.7) | 🔴 最低分，conv_path -5.8，阻融 -166%，全崩 |
| 万通发展 | 多 | **空** (29.9) | v1=4 静态合格，v3 判派发 |

Top3（好） vs Bot3（差）在 6 项演化维度的差异：

| 维度 | Top3 平均 | Bot3 平均 | 差距倍数 |
|:---|---:|---:|:---:|
| 主峰迁移（%） | **+18.6** | +1.3 | 15× |
| TPC连续天数 | **55 天** | 34 天 | 1.6× |
| 形态转移分 | **+0.7** | **-4.8** | 反向 |
| 阻力消融（%）| **+6.1** | **-47.1** | 反向 |
| 多周期一致 | **+0.7** | **-1.3** | 反向 |

→ 这 6 项维度的极差是 v3 区分"真强"与"假强"的核心依据。

### 4.8 今日分析决策（基于 v3 优先）

| 决策 | 股票数 | 代表 |
|:---|:---:|:---|
| ✅ 强烈持有 | 1 | **TCL科技** (v3=65.3) |
| ✅ 持续持有 | 8 | 立昂微、海思科、中国长城、领益智造、恒瑞医药、万通发展、美埃科技、中科曙光 |
| 🟡 观望持有 | 4 | 东鹏饮料、巨化股份、浪潮信息、双环传动 |
| 🔴 建议减仓 | 6 | 大元泵业、横店东磁、洪田股份、璞泰来、能科科技、宏发股份 |

### 4.9 每日日志已扩展字段

`daily_log.py` 的 `add_batch` / `add_stock` 现在自动保存 7 项演化指标 + v3 + v3_grade：

```python
{
  'v3', 'v3_grade',
  'peak_shift_14d', 'peak_shift_streak', 'tpc_streak',
  'conv_path_score', 'dense_days', 'res_melt_7d', 'evol_consistency',
}
```
...
| 优先级 | 任务 | 说明 |
|:---|:---|:---|
| P0 | **v3 权重校准** | 用 1337 笔真实交易 / 326 股回测做 SLSQP + CV，确认 v3 7 项权重是否需要调整（当前为手工初设） |
| P1 | **v3 纳入 total_score 公式** | 当前 total 建议 30%v1 + 30%v2 + 40%v3（v3 权重最高以对齐"演化为主"），需回测验证 |
| P2 | **新增阻力_支撑路径图** | 把 14 天主峰路径 / 阻力峰消融率画成可视化曲线（在 analysis.html 中）|
| P3 | **v3 接入 evolution_pipeline Layer0** | 把 Layer0 后验从 v1≥4 改为 v1≥4 + v3≥48 双条件，过滤"静态假强" |
| P4 | 月度 re-fit | 每月用新增交易数据 re-fit v3 权重，应对漂移 |
